...
_.forEach(skills, function (key, value) {
  skillsList = _.concat(skillsList, skills[value]);
});
...
var sortedByFriends = _.sortBy(data, ['friends', 'name']);
var fullySortedByFriends = _.map(sortedByFriends, 'name');

console.log('Names sorted by friends quantity:', fullySortedByFriends);
...
_.forEach(friends, function(key,value){ 
  friendsList = _.concat(friendsList, friends[value]);
});

var fullFriendsList = _.map(friendsList, 'name');
var uniqueFriends = _.uniq(fullFriendsList);

console.log('Full list of unique friends:', uniqueFriends);